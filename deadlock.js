		...
		{	
			...
			for(var i=0;i<index;i++)
			{
				...
				for(var j=0;j<num;j++)
				{
					...
					inp1.setAttribute("style","width:50px;text-align:center;margin-right:50px;float:left; border-radius: 20px;  border: 2px solid deeppink; padding:10px;  width: 40px;  height: 10px;");
			...
			p2.textContent = "Enter Need Matrix ( REQUESTED=1 , NOT REQUESTED=0 )";
			d2.appendChild(p2);
			var br=document.createElement("br");
			for(var i=0;i<index;i++)
			{
...
function detection()
{
	...
	for(var i=0;i<num;i++)
	{
		...
		for(var j=0;j<index;j++)
		{
			...
			if(c>1)
			{
				window.alert("Resource "+(j+1) +" cannot be allocated to more than one process");
				return;
			}
		}
